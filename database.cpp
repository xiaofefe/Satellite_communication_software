...
bool database::UpdataDatabase(QString Tasknumber1,QString sendingpointfre1, QString modulationstyle1,int sendingrate1,QString receivepointfre1,QString demodulationstyle1,int receiveingrate1)
{
    db = QSqlDatabase::database("sqlite1"); //建立数据库连接
    QSqlQuery query(db);
    query.exec(QString("UPDATE TaskDaiHaoTable SET sendingpointfre=:sendingpointfre1,modulationstyle=:modulationstyle1,sendingrate=:sendingrate1,"
                       "receivepointfre=:receivepointfre1,demodulationstyle=:demodulationstyle1,receiveingrate=:receiveingrate1 where tasknumber='%1'").arg(Tasknumber1));
